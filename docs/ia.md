...
> **Status:** Fase 1 ✅ implementada e verificável no código · Fase 2 🔍 proposta aguardando decisão do grupo.
...
## 3. Fase 2 — Jargões por área via consulta ao Supabase 🔍 (proposta)
...
**Migration proposta** (`supabase/migrations/0002_market_jargons.sql`):

```sql
create table public.market_jargons (
  id uuid primary key default gen_random_uuid(),
  area text not null,                -- ex.: 'administrativo', 'dados', 'desenvolvimento'
  role_hint text,                    -- opcional: cargo específico dentro da área
  terms text[] not null,             -- os jargões em si
  usage_note text,                   -- orientação de uso p/ IA (ex.: "não usar p/ nível júnior")
...
**Seed inicial:** 8–10 áreas comuns (administrativo, atendimento, dados, desenvolvimento, design, marketing, financeiro, RH, logística), ~10–15 termos cada, curados pelo time.
...
- [ ] Tabela `market_jargons` criada com RLS e seed de pelo menos 8 áreas
- [ ] Análise de área coberta injeta os termos no prompt (real e mock)
- [ ] `market_language_terms` das recomendações passa a refletir os termos curados quando aplicável
- [ ] Área sem jargões cadastrados: análise funciona exatamente como hoje (fallback silencioso)
- [ ] Nenhuma violação das regras de autenticidade (termo só entra se a experiência sustentar)

### 3.5 O que o time precisa decidir (avaliação)

| # | Decisão | Recomendação |
| --- | --- | --- |
| 1 | Implementar a Fase 2 agora ou após validar a Fase 1 com usuários? | Implementar — esforço baixo e é demonstrável na próxima mentoria |
| 2 | Degrau A (lookup) ou B (tool use)? | **A** — mais simples, mesma precisão para chave conhecida; B fica documentado como evolução |
| 3 | Quem cura a lista de jargões por área? | 1 dono por área no time (bom trabalho para os POs, direto no painel do Supabase) |
| 4 | Quando evoluir para o degrau C (pgvector)? | Somente quando existir base grande não estruturada (ex.: banco de vagas reais) |
...
timeline
    ...
    Fase 2 (proposta) : Jargões por área via Supabase : Injeção no prompt : Curadoria pelo time
